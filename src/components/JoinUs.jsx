...
import { getMyPostulation, createPostulation, deleteMyPostulation } from "../services/api";
...
function JoinUs() {
  const [form, setForm] = useState({ carrera_especialidad: "", motivacion: "" });
  ...
  useEffect(() => {
    ...
    getMyPostulation()
      .then((data) => setMyPostulation(data))
      .catch(() => setMyPostulation(null));
  ...
  const handleSubmit = async (e) => {
    ...
    setSuccess("");
    try {
      await createPostulation(form);
      setSuccess("¡Postulación enviada exitosamente!");
      setMyPostulation(form);
    } catch (err) {
  ...
  const handleDelete = async () => {
    if (!myPostulation?.id) return;
    setLoading(true);
    setError("");
    setSuccess("");
    try {
      await deleteMyPostulation(myPostulation.id);
      ...
      <div className="joinus-container">
        ...
        <div className="joinus-main">
          ...
          <div className="joinus-form-box">
            {myPostulation ? (
              <div className="joinus-postulation-status">
                <h3>Ya enviaste tu postulación</h3>
                <p><b>Carrera/Especialidad:</b> {myPostulation.carrera_especialidad}</p>
                <p><b>Motivación:</b> {myPostulation.motivacion}</p>
                <p><b>Estado:</b> {myPostulation.estado || "pendiente"}</p>
                <button className="joinus-btn" onClick={handleDelete} disabled={loading}>
                  {loading ? "Eliminando..." : "Eliminar postulación"}
                </button>
                {success && <div style={{ color: "#00796b", marginTop: 8 }}>{success}</div>}
                {error && <div style={{ color: "#d93240", marginTop: 8 }}>{error}</div>}
              </div>
            ) : (
              <form className="joinus-form" onSubmit={handleSubmit}>
                <input
                  ...
                  value={form.carrera_especialidad}
                ...
                <textarea
                  ...
                  onChange={handleChange}
                />
                <button type="submit" className="joinus-btn" disabled={loading}>
        ...
        <div className="joinus-social-final">
          ...
          <div className="joinus-social-icons">
            <a href="https://www.instagram.com/lead_peru/" target="_blank" rel="noopener noreferrer" className="joinus-social-icon instagram">
            ...
            <a href="https://www.linkedin.com/company/leadmindsetorg/posts/?feedView=all" target="_blank" rel="noopener noreferrer" className="joinus-social-icon linkedin">
              <svg width="24" height="24" fill="white" viewBox="0 0 24 24">
