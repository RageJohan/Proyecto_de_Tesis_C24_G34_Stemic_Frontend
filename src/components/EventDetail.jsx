        ...
        <div className="event-detail-main-box">
          ...
          <div className="event-detail-info-box">
            ...
            <div className="event-detail-table">
              <div><b>Fecha del evento y hora:</b> <span>{date ? new Date(date).toLocaleString() : ""}</span></div>
              <div><b>Duración:</b> <span>{duration}</span></div>
              <div><b>Modalidad:</b> <span>{modalidad}</span></div>
              <div><b>Espacio de realización:</b> <span>{espacio}</span></div>
              <div><b>Requiere postulación:</b> <span>{requierePostulacion ? "Sí" : "No"}</span></div>
              <div><b>Consultas aquí:</b> <span>{contacto}</span></div>
            </div>
            <div className="event-detail-tags">
              {tags.map((tag, i) => (
                <span className="event-detail-tag" key={i}>{tag}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="event-detail-section">
          ...
          <div className="event-detail-skills">
            {skills.length > 0 ? skills.map((skill, i) => (
              <span className="event-detail-skill" key={i}>{skill}</span>
            )) : <span style={{ color: '#888' }}>No especificado</span>}
          </div>
        </div>
        <button className="event-detail-back" onClick={() => navigate(-1)}>Volver</button>
      </div>
    </>
  );
}
